...
_ACTIVE_GIT_QUERY_CACHE: ContextVar[
    dict[tuple[str, str, tuple[str, ...]], int | str | bytes | None] | None
] = ContextVar("_ACTIVE_GIT_QUERY_CACHE", default=None)


@dataclass(frozen=True)
class _GitQuerySession:
    repo_root: Path
    before: tuple[bytes, bytes, bytes, bytes]
    token: Token[
        dict[tuple[str, str, tuple[str, ...]], int | str | bytes | None] | None
    ]
...
def _git_return_code(repo_root: Path, arguments: Sequence[str]) -> int | None:
    ...
    if command is None:
        return None
    cache = _ACTIVE_GIT_QUERY_CACHE.get()
    cache_key = (
        "return-code",
...
def _git_repository_validation_snapshot(
    repo_root: Path,
) -> tuple[bytes, bytes, bytes, bytes] | None:
    """Capture the Git-visible state that may change a cached validation answer."""
    ...
    values = tuple(_git_bytes(repo_root, query) for query in queries)
    if len(values) != 4 or any(value is None for value in values):
        return None
    return values  # type: ignore[return-value]


def _begin_git_query_session(repo_root: Path) -> _GitQuerySession | None:
    if _ACTIVE_GIT_QUERY_CACHE.get() is not None:
        return None
    before = _git_repository_validation_snapshot(repo_root)
    if before is None:
        return None
    token = _ACTIVE_GIT_QUERY_CACHE.set({})
    return _GitQuerySession(repo_root=repo_root, before=before, token=token)


def _finish_git_query_session(session: _GitQuerySession) -> bool:
    _ACTIVE_GIT_QUERY_CACHE.reset(session.token)
    after = _git_repository_validation_snapshot(session.repo_root)
    return after is not None and after == session.before
...
def _parse_added_paths(raw: bytes | None) -> set[str] | None:
    if raw is None:
        return None
...
def _read_git_blobs_batch(
    ...
    if not requested or any(
        re.fullmatch(r"[0-9a-f]{40}", value) is None for value in requested
    ):
        return None
    output = _git_bytes_with_input(
        repo_root,
    ...
    for expected in requested:
        ...
        if (
            ...
            return None
        result[object_id] = output[content_start:content_end]
        offset = content_end + 1
    return result if offset == len(output) else None


def _git_attributes_are_raw_safe_batch(
    repo_root: Path,
    relative_paths: Sequence[str],
    problems: Problems,
    code_prefix: str,
    ...
    if not paths or len(set(paths)) != len(paths):
        problems.add(
            f"{code_prefix}_ATTRIBUTES",
            str(repo_root),
            "raw attribute input must be a non-empty unique path list",
        )
        return False
    expected = {
        "text": "unset",
    ...
    if fields and fields[-1] == b"":
        fields.pop()
    if len(fields) != len(paths) * len(expected) * 3:
        valid = False
    else:
        for index in range(0, len(fields), 3):
            try:
            ...
            observed[attribute] = value
    invalid_paths = [
        path for path in paths if observed_by_path.get(path) != expected
    ]
    if invalid_paths:
        valid = False
    if not valid:
        problems.add(
            f"{code_prefix}_ATTRIBUTES",
            invalid_paths[0] if invalid_paths else str(repo_root),
            "paths must be -text with executable content transforms unspecified",
        )
...
def validate_bootstrap_control_plane(
    ...
    paths = list(BOOTSTRAP_CONTROL_PATHS)
    path_set = set(paths)
    tracked = _git_stdout(
        repo_root, ("ls-files", "--error-unmatch", "--", *paths)
    )
    dirty = _git_bytes(
        repo_root,
    ...
    add_history = _git_stdout(
        repo_root,
        ("log", "--diff-filter=A", "--format=%H", "--reverse", "--", *paths),
    )
    all_history = _git_stdout(
        repo_root, ("log", "--format=%H", "--reverse", "--", *paths)
    )
    tracked_paths = set((tracked or "").splitlines())
    add_commits = [value for value in (add_history or "").splitlines() if value]
    history_commits = [value for value in (all_history or "").splitlines() if value]
    if (
        tracked is None
        or dirty is None
        or add_history is None
        or all_history is None
        or tracked_paths != path_set
        or len(add_commits) != 1
    ):
        problems.add(
            "BOOTSTRAP_CONTROL_GIT",
            str(repo_root),
            "bootstrap controls require one readable shared first-add history",
        )
        return
    first_commit = add_commits[0]
    added_paths = _parse_added_paths(
        _git_bytes(
            repo_root,
            (
                "diff-tree",
    ...
    first_inventory = _parse_tree_inventory(
        _git_bytes(repo_root, ("ls-tree", "-r", "-z", first_commit, "--", *paths))
    )
    head_inventory = _parse_tree_inventory(
        _git_bytes(repo_root, ("ls-tree", "-r", "-z", "HEAD", "--", *paths))
    )
    index_inventory = _parse_index_inventory(
        _git_bytes(repo_root, ("ls-files", "--stage", "-z", "--", *paths))
    )
    if added_paths != path_set:
        problems.add(
            "BOOTSTRAP_CONTROL_HISTORY",
            str(repo_root),
            "every bootstrap control must have status A in the shared first-add commit",
        )
    if history_commits != [first_commit]:
        problems.add(
            "BOOTSTRAP_CONTROL_HISTORY_MUTATION",
            str(repo_root),
            "bootstrap controls changed after their shared first-add commit",
        )
    if dirty:
        problems.add(
            "BOOTSTRAP_CONTROL_IMMUTABLE",
            str(repo_root),
            "bootstrap control worktree paths must be clean",
        )
    if (
        first_inventory is None
        or head_inventory is None
        or index_inventory is None
        or set(first_inventory) != path_set
        or set(head_inventory) != path_set
        or set(index_inventory) != path_set
    ):
        problems.add(
            "BOOTSTRAP_CONTROL_GIT",
            str(repo_root),
            "bootstrap tree/index inventories must exactly cover the frozen path set",
        )
        return
    object_ids = [first_inventory[path][1] for path in paths]
    blob_raw = _read_git_blobs_batch(repo_root, object_ids)
    if blob_raw is None:
        problems.add(
            "BOOTSTRAP_CONTROL_GIT",
            str(repo_root),
            "bootstrap blobs could not be read in one bounded batch",
        )
        return
    for relative_path in paths:
        first_mode, first_object = first_inventory[relative_path]
        head_mode, head_object = head_inventory[relative_path]
        index_mode, index_object = index_inventory[relative_path]
        try:
            worktree_path = repo_root / relative_path
            worktree_raw = worktree_path.read_bytes()
            worktree_unsafe = _path_has_reparse_component(
                repo_root, relative_path
            )
        except OSError:
            worktree_raw = None
            worktree_unsafe = True
        first_raw = blob_raw.get(first_object)
        if (
            first_raw is None
            or head_object != first_object
            or index_object != first_object
            or worktree_raw != first_raw
        ):
            problems.add(
                "BOOTSTRAP_CONTROL_IMMUTABLE",
    ...
    _git_attributes_are_raw_safe_batch(
        repo_root, paths, problems, "BOOTSTRAP_CONTROL"
    )
...
def _validate_w84_g0_overlay_impl(
    *,
    ...
    repository_context: Any = None
    git_query_session: _GitQuerySession | None = None
    ...
    try:
        ...
        if (
            ...
            raise trusted_executor.ExecutorError("GIT_WORKTREE_TOPOLOGY")
        git_query_session = _begin_git_query_session(candidate)
        if git_query_session is None:
            raise trusted_executor.ExecutorError("GIT_QUERY_SESSION")
    ...
    if candidate is None or control is None or repository_context is None:
        if git_query_session is not None:
            try:
                if not _finish_git_query_session(git_query_session):
                    reject("OVERLAY_REPOSITORY_DRIFT")
    ...
    try:
        if git_query_session is None or not _finish_git_query_session(
            git_query_session
        ):
            reject("OVERLAY_REPOSITORY_DRIFT")
        git_query_session = None
...
def validate_w84_g0_overlay(
    *,
    ...
    previous_cache = _ACTIVE_GIT_QUERY_CACHE.get()
    try:
        ...
        # An unexpected exception may bypass the normal session close.  Never
        # leak request-scoped results into a later validation.
        _ACTIVE_GIT_QUERY_CACHE.set(previous_cache)
