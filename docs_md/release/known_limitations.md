...
- Week 58-62 Project Pack contracts, discovery/plan, managed staging/checkpoint, controlled conversion, TIFF verification, and managed preview/report evidence are 0.5.0 source-only Preview work. They are not included in the accepted 0.4.0 artifact and do not change the 0.4.0 release decision.
...
- The Gerber/TIFF project pack now records deterministic plan/staging/checkpoint, controlled Gerbv/ImageMagick conversion, bounded TIFF verification, managed preview/report evidence, and run/job/artifact correlation. The complete workflow is still Preview because human accept/reject, execute restart, and artifact lifecycle commands remain Deferred.
...
- `packs resume --dry-run` is eligibility diagnostics only. `staged`/`ready` requires new approval before execution; `running`/`interrupted` requires an explicit restart decision and is never auto-replayed. `verifying` is handled by the explicit Week 62 verifier, and `awaiting-acceptance` can continue only at the future human gate. There is still no execute restart command.
...
- Default tests use protocol-v1 fake driver contracts plus the bundled Magick.NET/ImageMagick managed codec on synthetic fixtures; they do not require installed Gerbv/`magick.exe`, a model, credentials, network, or real manufacturing data. Fake success/metadata/preview is never real-tool acceptance. Real conversion/verification smoke remains explicit opt-in through `CAICLI_GERBER_TIFF_TOOL_SMOKE=1` plus caller-provided tool paths.
