...
- Week 50 does not provide job retention, rotation, delete, cleanup, or workspace relocation commands. Records remain in the user-level store until the user removes them, and local paths in records/exports should still be treated as sensitive metadata.
- Task queue execution, retry/cancel queue state, automation schedules, multi-role pipelines, CI/PR providers, daemon/API/SSE, and remote job control remain Deferred.
