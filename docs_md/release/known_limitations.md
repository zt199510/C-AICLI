...
- Local task queue v1 is manual and single-process oriented. It has no daemon, scheduler, concurrent worker pool, lease/heartbeat recovery, remote runner, or automatic recovery for a process terminated while an item is running.
...
- Automation `schedule` fields are preview and validation data only. There is no background scheduler, Windows Task Scheduler registration, daemon worker, automatic retry/resume, webhook, remote trigger, or team automation. CI/PR providers, daemon/API/SSE, and remote job control remain Deferred.
