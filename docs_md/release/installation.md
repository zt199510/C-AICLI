...
The first release candidate is a Windows `win-x64` self-contained package produced by:
...
```text
artifacts/release/caicli-0.1.0-win-x64/caicli.exe
```
...
- An OpenAI API key only for real `chat` model calls. `doctor`, `config get`, `version`, MCP diagnostics, workflow diagnostics, and smoke tests can run without a key.
...
```powershell
$env:PATH = "$(Resolve-Path artifacts\release\caicli-0.1.0-win-x64);$env:PATH"
caicli version
caicli doctor
```
