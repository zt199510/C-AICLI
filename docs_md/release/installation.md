...
The `0.2.0` release candidate is a Windows `win-x64` self-contained package produced by:
...
```text
artifacts/release/caicli-0.2.0-win-x64/caicli.exe
```
...
```powershell
$env:PATH = "$(Resolve-Path artifacts\release\caicli-0.2.0-win-x64);$env:PATH"
caicli version
caicli doctor
```
