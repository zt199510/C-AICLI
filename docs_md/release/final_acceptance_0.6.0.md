...
| G7 CLI regression/version | Pending new clean revision | 重复出现 Windows manifest 原子覆盖瞬时争用；已做限定于 sharing/access-denied 的有界重试并补真实 reader-lock 回归，需新 revision 全量重跑。 |
