...
| `caicli exec` | Solidified | Agentic v1 surface routed through `IAgentRunner`; emits model/tool/final/error events with terminal `status`/`stopReason`, supports `--cwd` instruction target selection, `--output text|json`, `--max-steps`/`--max-turns`, `--max-tool-calls`, `--max-retries`, timeout budgets, approval-gated tool actions, session transcript run summaries, and `--resume` context. Tool/shell/verification failures can be summarized as bounded model feedback for finite retry, and retry/failure summaries preserve changed files, commands, and stop reasons. Configured direct OpenAI runs use real Responses SDK tool-call continuation through the same offline/fake agent contract. |
