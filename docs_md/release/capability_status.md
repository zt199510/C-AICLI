...
| `caicli exec` | Solidified | Agentic v1 surface routed through `IAgentRunner`; emits model/tool/final/error plus read-only `review.gate` and final `taskReport` events with terminal `status`/`stopReason`, supports `--cwd` instruction target selection, inline bounded workflow references, `--output text|json`, `--max-steps`/`--max-turns`, `--max-tool-calls`, `--max-retries`, timeout budgets, approval-gated tool actions, session transcript run summaries, and `--resume` context. Tool/shell/verification failures can be summarized as bounded model feedback for finite retry, and retry/failure summaries preserve changed files, commands, and stop reasons. Task reports preserve references, changed files, commands, verification status, remaining risks, trace path, and secret presence/source/kind without storing secret values. Configured direct OpenAI runs use real Responses SDK tool-call continuation through the same offline/fake agent contract. |
...
| Standalone markdown task report file | Deferred | `exec` emits compact text/JSON/trace/session `taskReport` data; always writing a separate full markdown report file is not enabled. |
| Expert profiles and local skills/workflow packs | Deferred | `--expert`, `skills list/run`, pack manifests, and built-in workflow packs are planned for later 0.3.x work and are not enabled by the 0.3.1 workflow-inputs/changes-view increment. |
