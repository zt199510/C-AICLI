...
- [x] Step 1: 汇总 Week 27-37 review 状态，列出未完成项。
- [x] Step 2: 更新 smoke tests，覆盖 0.2.0 CLI 新命令。
- [x] Step 3: 更新 version metadata 到 `0.2.0`。
- [x] Step 4: 更新 CHANGELOG。
- [x] Step 5: 更新 configuration、quickstart、security model、known limitations、capability status。
- [x] Step 6: 运行 `dotnet build src/CSharpAiCli.sln -c Release`。
- [x] Step 7: 运行 `dotnet test src/CSharpAiCli.sln -c Release --no-build`。
- [x] Step 8: 运行 `tools/Build-Release.ps1` 两次并确认 zip SHA256 deterministic。
- [x] Step 9: 运行 `tools/Invoke-SmokeTests.ps1`。
- [x] Step 10: 创建 `38_week_review.md` 和 `docs_md/release/final_acceptance_0.2.0.md`。
