...
| 11 | 2024-08-12 至 2024-08-18 | 阶段 03 | 计划中 | 添加 patch applier、dirty workspace 检测和文件编辑审批流程。 | patch 可预览、审批、应用；目标文件预览后变化会拒绝应用。 |
| 12 | 2024-08-19 至 2024-08-25 | 阶段 03 | 计划中 | 添加 shell runner、命令审批、危险命令阻断、超时和输出截断。 | 无害命令审批后运行；危险模式、cwd 越界和超时路径有测试。 |
| 13 | 2024-08-26 至 2024-09-01 | 阶段 03 | 计划中 | 添加 git 工具并完成第一条读取、编辑、测试 MVP 工作流。 | 阶段 03 验收清单通过；git 测试使用临时仓库。 |
| 14 | 2024-09-02 至 2024-09-08 | 阶段 04 | 计划中 | 添加 Microsoft Agent Framework 适配器项目或命名空间。 | 适配器构建成功，且不需要修改 CLI 命令代码；direct 后端保持可用。 |
| 15 | 2024-09-09 至 2024-09-15 | 阶段 04 | 计划中 | 将本地工具桥接到 Microsoft Agent Framework 后端。 | 框架后端可以调用读取和搜索工具。 |
| 16 | 2024-09-16 至 2024-09-22 | 阶段 04 | 计划中 | 添加后端配置开关和回退诊断。 | direct 和 framework 后端都可以通过配置选择；framework 缺失时 doctor 解释原因。 |
| 17 | 2024-09-23 至 2024-09-29 | 阶段 04 | 计划中 | 回归测试两个后端，并记录框架锁定边界。 | 阶段 04 验收清单通过，或作为增强目标清晰标注未启用。 |
| 18 | 2024-09-30 至 2024-10-06 | 阶段 05 | 计划中 | 添加 MCP 配置模型和 `mcp list` 命令。 | MCP 配置可加载，已禁用 servers 保持 inactive。 |
| 19 | 2024-10-07 至 2024-10-13 | 阶段 05 | 计划中 | 添加 MCP 连接管理器和 `mcp doctor`。 | 已配置 servers 的连接诊断可用。 |
| 20 | 2024-10-14 至 2024-10-20 | 阶段 05 | 计划中 | 将 MCP 工具桥接到通用工具注册表。 | MCP 工具在启用后可列出并调用。 |
| 21 | 2024-10-21 至 2024-10-27 | 阶段 05 | 计划中 | 添加项目工作流注册表和验证 profiles。 | 工作流可以建议已配置的验证命令；路径来自 profile 或 `--workspace`。 |
| 22 | 2024-10-28 至 2024-11-03 | 阶段 05 | 计划中 | 添加面向计划文档和 C++ 验证的 Gerber/TIFF 工作流包。 | 阶段 05 验收清单通过，或作为增强目标清晰标注未启用。 |
| 23 | 2024-11-04 至 2024-11-10 | 阶段 06 | 计划中 | 添加发布构建脚本和版本元数据。 | 发布构建命令会产生产物。 |
| 24 | 2024-11-11 至 2024-11-17 | 阶段 06 | 计划中 | 添加安装、配置、安全、MVP/增强能力状态和快速开始文档。 | 新用户可以按照文档运行 `doctor` 和 `chat`。 |
| 25 | 2024-11-18 至 2024-11-24 | 阶段 06 | 计划中 | 添加 smoke tests 和发布加固修复。 | Smoke test 脚本在干净测试工作区通过，覆盖缺 key、审批拒绝、路径越界和工具禁用。 |
| 26 | 2024-11-25 至 2024-12-01 | 阶段 06 | 计划中 | 最终验收、changelog、已知限制和第一个发布包。 | 半年验收清单通过。 |
