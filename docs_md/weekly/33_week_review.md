...
- `review` 保持只读：不执行 patch/shell/tools，不写 transcripts/logs/files/patches；真实使用仍需有效模型凭据。
- diff 收集路径完成只读加固：避免 external diff、textconv、clean filter、index refresh 等副作用；unsupported metadata/non-regular tracked entries 会用 summary/warning 命名路径，而不是修改 repo 状态或静默漏报。
...
运行时说明：
- `review` 是只读审查入口，不写 logs/transcripts/patches/files，也不运行 shell/patch tools；但会把当前 git diff 发送给 configured model provider，真实使用需要有效模型配置、凭据与网络/配额。
- `diff` 使用 hardened read-only collection path；对 unsupported metadata/non-regular tracked entries 会输出 summary/warning 并命名路径，而不是为生成 diff 去变更 repo 状态。
