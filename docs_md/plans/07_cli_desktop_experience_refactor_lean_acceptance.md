...
- W92 候选仅运行一次完整 Desktop verify、一次完整 .NET test，以及必要的最终冒烟。
...
- 单个改动切片的自动验证目标不超过 15 分钟。
- 单个里程碑自动验证目标不超过 30 分钟。
- 命令超过预算、挂起或重复失败时立即停止，记录具体失败并回到代码定位；不自动扩大到更重的测试矩阵。
...
阶段最终完成仍要求 W92 的一次完整回归和用户视觉验收，但不要求恢复 104 Gate 证据流水线。
