import { FilePlus2, FolderPlus, RefreshCw, Send, Square, Trash2, X } from "lucide-react";
    ...
    <section className="composer" aria-label="Composer">
      ...
      <div className="composer-footer">
        ...
        <div className="composer-summary">
          <span>{props.composer.snapshot ? `${props.composer.snapshot.effectiveModel} · ${props.composer.snapshot.approvalMode}` : props.modelLabel ? `${props.modelLabel}${props.approvalModeLabel ? ` · ${props.approvalModeLabel}` : ""}` : "Model unavailable"}</span>
          {stopMode ? (
            <button className="composer-send composer-stop" type="button" onClick={props.onStop} disabled={props.stopping} aria-label={props.stopping ? "Stopping response" : "Stop response"}><Square size={15} fill="currentColor" /><span className="sr-only">{props.stopping ? "Stopping" : "Stop"}</span></button>
          ) : props.disabledActionLabel ? (
            <button className="composer-send composer-alternate-action" type="button" onClick={props.onDisabledAction} disabled={!props.onDisabledAction} aria-label={props.disabledActionLabel}><RefreshCw size={17} /><span className="sr-only">{props.disabledActionLabel}</span></button>
          ) : (
            <button className="composer-send" type="button" onClick={props.onSend} disabled={disabled || !props.draft.text.trim()} aria-label={busy ? "Sending prompt" : "Send prompt"}><Send size={17} /><span className="sr-only">{busy ? "Sending" : "Send"}</span></button>
          )}
        </div>
      </div>
      <div id="composer-help" className="sr-only">Enter sends. Shift Enter inserts a new line.</div>
      <div id="composer-status" className={props.draft.error ? "composer-error" : "composer-status"} role={props.draft.error ? "alert" : "status"} hidden={!statusText}><span>{statusText}</span></div>
    </section>
  );
}
