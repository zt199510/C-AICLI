  ...
  return (
    <div className="thread-navigation">
      <div className="thread-actions">
        <select aria-label="Filter conversations" value={filter} onChange={(event) => setFilter(event.target.value as Filter)}>
        ...
        <button className="new-thread-button" type="button" aria-label="New conversation" title="New conversation" onClick={props.onNew}>
          <Plus size={15} aria-hidden="true" /> New
        </button>
      </div>
      {commandError && <div className="inline-error" role="alert">{commandError}</div>}
      {props.truncated && <div className="capped-banner">Showing the first 200 conversations.</div>}
      {props.status === "loading" && props.threads.length === 0 ? <div className="empty-list">Loading conversations…</div> : null}
      {props.error ? <div className="inline-error" role="alert">{props.error}</div> : null}
      {props.status !== "loading" && visible.length === 0 ? <div className="empty-list">No conversations match this view.</div> : null}
          ...
          <div className={`thread-row ${props.selectedThreadId === entry.threadId ? "selected" : ""}`} role="listitem" key={entry.threadId}>
            {editing === entry.threadId ? (
              <form className="thread-form compact" onKeyDown={(event) => { if (event.key === "Escape") { event.preventDefault(); event.stopPropagation(); closeRename(entry.threadId); } }} onSubmit={(event) => {
                ...
                <label className="sr-only" htmlFor={`rename-${entry.threadId}`}>Rename conversation</label>
                <input id={`rename-${entry.threadId}`} autoFocus value={editTitle} maxLength={512} onChange={(event) => setEditTitle(event.target.value)} />
                <div className="form-actions"><button type="submit" aria-label="Save conversation title"><Check size={14} aria-hidden="true" /></button><button type="button" aria-label="Cancel rename" onClick={() => closeRename(entry.threadId)}><X size={14} aria-hidden="true" /></button></div>
              </form>
            ) : (
              <>
            ...
            {confirmArchive === entry.threadId && (
              <div className="archive-confirm" role="alertdialog" aria-modal="true" aria-labelledby={`archive-title-${entry.threadId}`} onKeyDown={(event) => { if (event.key === "Escape") { event.preventDefault(); event.stopPropagation(); closeArchive(entry.threadId); } }}>
                <span id={`archive-title-${entry.threadId}`}>Archive this conversation?</span>
